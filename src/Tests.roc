...
expect # TEST: InputAppName to PlatformSelect w/ empty buffer & existing appName in config
    initModel = Model.init [] []
    model = { initModel &
...
expect # TEST: InputAppName - appendToBuffer w/ illegal Key
    model = Model.init [] []
    newModel = model 
...
expect # TEST: InputAppName - backspaceBuffer
    model = Model.init [] []
    newModel = model
        |> Model.appendToBuffer LowerA
